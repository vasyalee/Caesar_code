def rus_encrypt(ru_str):
    ...
    for letter in ru_str:
        if letter.islower() and letter in rus_lower:
            res += rus_lower[rus_lower.find(letter) + k]
        elif letter.isupper() and letter in rus_upper:
            res += rus_upper[rus_upper.find(letter) + 35]
        else:
            res += letter
    return res
...
def rus_decrypt(ru_encrypted):
    ...
    for letter in ru_encrypted:
        if letter.islower() and letter in rus_lower:
            res += rus_lower[(rus_lower.find(letter) - k) % 32]
        elif letter.isupper() and letter in rus_upper:
            res += rus_upper[(rus_upper.find(letter) - k) % 32]
        else:
            res += letter
    return res
